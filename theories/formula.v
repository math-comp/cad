...
From mathcomp Require Import div tuple finfun generic_quotient bigop finset.
From mathcomp Require Import perm ssralg poly polydiv ssrnum mxpoly binomial.
From mathcomp Require Import finalg zmodp mxpoly mxtens qe_rcf ordered_qelim.
From mathcomp Require Import realalg matrix finmap order finset mpoly.
...
move=> d; apply/fsubsetP/idP; last first.
  ...
  rewrite (leq_trans leq_ca) // (leq_trans lt_xb)//.
move=> h.
apply/andP; split; [move/(_ a) : h | move/(_ (a + b)%N) : h]; rewrite !mnfsetE.
...
Local Lemma set_nth_size (T : Type) (d : T) (n : nat)
    (x : n.-tuple T) (i : 'I_n) (y : T) :
  size (set_nth d x i y) == n.
Proof. by rewrite size_set_nth size_tuple; apply/eqP/maxn_idPr. Qed.

Canonical set_nth_tuple (T : Type) (d : T) (n : nat)
    (x : n.-tuple T) (i : 'I_n) (y : T) :=
...
Lemma eval_big (op : term R -> term R -> term R) (x : term R) (I : Type)
    (r : seq I) (P : pred I) (F : I -> term R) (e : seq R) (eop : R -> R -> R) :
  (forall t u, eval e (op t u) = eop (eval e t) (eval e u))
  -> eval e (\big[op/x]_(i <- r | P i) F i)
    = \big[eop/eval e x]_(i <- r | P i) eval e (F i).
Proof.
move=> opE.
...
Definition term_mpoly n (P : {mpoly R[n]}) (t : 'I_n -> term R) : term R :=
  \big[Add/Const 0]_(m <- msupp P) Mul (Const P@_m)
    (\big[Mul/Const 1]_i Exp (t i) (m i)).

Definition eval_term_mpoly n (P : {mpoly R[n]})
    (t : 'I_n -> term R) (e : seq R) :
...
Lemma meval_mpoly_rterm (R : realDomainType) (n : nat)
    (x : 'I_n -> R) (t : term R) :
...
Lemma eqn_holds (n : nat) (e1 e2 : seq R) (f : {formula_n R}) :
  (\row_(i < n) (e1`_(val i)) =2 (\row_(i < n) e2`_(val i)))
  -> holds e1 f -> holds e2 f.
Proof.
move=> h; move/holds_take => h'.
...
Lemma holdsAnd (I : eqType) (r : seq I) (P : pred I)
    (e : seq R) (f : I -> formula R) :
...
Lemma holdsOr (I : eqType) (r : seq I) (P : pred I)
    (e : seq R) (f : I -> formula R) :
...
Fact elim_rformP (f : formula F) (e : seq F) :
  rformula f
  -> reflect (holds e f) (qf_eval e (@quantifier_elim _ (@wproj _) f)).
Proof.
...
Fact rcf_sat_Implies (e : seq F) (f g : formula F) :
  rcf_sat e (f ==> g) = ((rcf_sat e f) ==> (rcf_sat e g)).
Proof.
by apply/rcf_satP/implyP => /= hfg; move/rcf_satP => h; apply/rcf_satP/hfg.
Qed.

Fact rcf_sat_Not (e : seq F) (f : formula F) :
  rcf_sat e (~ f) = ~~ (rcf_sat e f).
Proof. by []. Qed.

(* TODO: generalize to sequences with `has`? *)
Lemma rcf_sat_exists [k : nat] (l : seq F) (E : 'I_k -> formula F) :
  rcf_sat l (\big[Or/False]_(i < k) E i) = [exists i, rcf_sat l (E i)].
Proof.
apply/rcf_satP/existsP => [/holdsOr|] /= [] x.
  by move=> [_][_] /rcf_satP Ex; exists x.
move=> /rcf_satP Ex; apply/holdsOr; exists x.
by split; first exact/mem_index_enum.
Qed.
...
Definition simp_rcf_sat :=
  (rcf_sat_Bool, rcf_sat_Equal, rcf_sat_Lt, rcf_sat_Le, rcf_sat_Unit,
  rcf_sat_And, rcf_sat_Or, rcf_sat_Implies, rcf_sat_Not).
...
Lemma formula_fv_bigAnd (I : Type) (r : seq I)
    (P : pred I) (E : I -> formula F) :
  formula_fv (\big[And/True%oT]_(i <- r | P i) (E i)) =
  \bigcup_(i <- r | P i) (formula_fv (E i)).
Proof. exact: big_morph. Qed.

Lemma formula_fv_bigOr (I : Type) (r : seq I)
    (P : pred I) (E : I -> formula F) :
...
Lemma forall_formulan (i : nat) (f : {formula_n F}) :
  nvar n ('forall 'X_i, f)%oT.
Proof.
by rewrite /nvar (fsubset_trans (@fsubD1set _ _ _)) // fsubset_formulan_fv.
Qed.
...
Lemma And_formulan (I : finType) (x : {formula_n F}) (r : seq I)
    (P : pred I) (f : I -> {formula_n F}) :
...
Canonical Structure formulan_And (I : finType) (x : {formula_n F})
    (r : seq I) (P : pred I) (f : I -> {formula_n F}) :=
  MkFormulan (And_formulan x r P f).

Lemma Or_formulan (I : finType) (x : {formula_n F}) (r : seq I)
    (P : pred I) (f : I -> {formula_n F}) :
...
Canonical Structure formulan_Or (I : finType) (x : {formula_n F})
    (r : seq I) (P : pred I) (f : I -> {formula_n F}) :=
...
split => h e; (split; first exact: holds_forall).
  ...
  exact/(rwP (elim_rformP _ rform_f))/(rwP (qf_evalP _ (qf_form_elim rform_f))).
move/(rwP (qf_evalP _ (qf_form_elim rform_f)))/(rwP (elim_rformP _ rform_f)).
move/h; apply: monotonic_forall_if => e2 h2.
exact/(rwP (qf_evalP _ (qf_form_elim rform_f)))/(rwP (elim_rformP _ rform_f)).
Qed.
...
- move=> t1 t2 s; rewrite fsubUset /=.
  apply/andP; split; rewrite (fsubset_trans (fv_tsubst_map k _ _)) //;
    apply/seq_fset_sub/sub_map_filter => i; rewrite in_fsetU => -> //.
  by rewrite orbT.
- move=> t1 t2 s; rewrite fsubUset /=.
  apply/andP; split; rewrite (fsubset_trans (fv_tsubst_map k _ _)) //;
    apply/seq_fset_sub/sub_map_filter => i; rewrite in_fsetU => -> //.
...
- move=> f1 h1 f2 h2 s /=; rewrite fsubUset.
  apply/andP; split.
...
Fact fv_subst_formula_map (k : unit) (s : seq nat) f :
  ...
  seq_fset k [seq nth O s i | i <- (iota O (size s)) & (i \in formula_fv f)].
Proof.
rewrite /subst_formula (fsubset_trans (fv_qf_subst_formula k _ _)) //.
apply/seq_fset_sub/sub_map_filter => i.
...
Definition cut (n : nat) (f : formula F) := subst_formula (iota 0 n) f.
...
Lemma rterm_tsubst (R : unitRingType) (t : GRing.term R)
    (s : nat * GRing.term R) :
...
Lemma rform_fsubst (R : realDomainType) (f : formula R)
    (s : nat * GRing.term R) :
...
Lemma rform_qf_elim (f : formula F) : rformula (qf_elim f).
Proof.
rewrite /qf_elim.
