...
From mathcomp Require Import fintype generic_quotient div tuple bigop ssralg.
From mathcomp Require Import poly polydiv finmap matrix mxpoly ssrnum order set.
...
Fact set_nthS (e : seq T) (i : nat) (x y : T) :
  size e <= i -> set_nth x e i y = set_nth x (rcons e x) i y.
Proof.
...
Lemma set_nth_nrcons (e : seq T) (i : nat) (x y : T) :
  size e <= i -> (set_nth x e i y) = rcons (nrcons (i - (size e)) x e) y.
Proof.
move: {2}(i - size e)%N (erefl (i - size e))%N x y => n.
...
Lemma set_nth_over (e : seq T) (i : nat) (x y : T) : 
  size e <= i -> (set_nth x e i y) =
                     rcons (e ++ (nseq (i - (size e)) x)) y.
Proof. 
by move=> h; rewrite set_nth_nrcons //; congr rcons; rewrite nseq_cat.
Qed.

Lemma set_nth_nseq (i j : nat) (x y z : T) : 
  i <= j -> set_nth x (nseq j y) i z = (rcons (nseq i y) z) ++ (nseq (j - i).-1 y).
Proof.
...
Lemma set_nth_rcons (i : nat) (e : seq T) (a x y : T) : 
  i < size e -> set_nth a (rcons e y) i x = rcons (set_nth a e i x) y.
Proof.
move: i x y; elim: e => //.
...
by rewrite cat_rcons -[i - size e1]prednK ?subn_gt0 // subnS. 
Qed.

Lemma set_nth_catl (i : nat) (e1 e2 : seq T) (x y : T) : 
  i < size e1 -> set_nth x (e1 ++ e2) i y = set_nth x e1 i y ++ e2.
Proof.
...
Lemma set_nth_cat (i : nat) (e1 e2 : seq T) (x y : T) :
  set_nth x (e1 ++ e2) i y = if i < size e1 then set_nth x e1 i y ++ e2
...
Lemma take_rcons (i : nat) (e : seq T) (x : T) :
  take i (rcons e x) = if i <= size e then take i e 
...
Lemma set_nth_takeC (i : nat) (e : seq T) (j : nat) (x y : T) : 
  j < i -> set_nth y (take i e) j x = take i (set_nth y e j x).
Proof.
move=> lt_ji.
...
Lemma set_nth_take (i : nat) (e : seq T) (j : nat) (x y : T) : 
  i <= j -> 
...
Fact perm_eq1 (x : T) : (perm_eq s [::x]) = (s == [::x]).
Proof.
apply/idP/eqP => [h|->]; last exact: perm_eq_refl.
exact: perm_eq_small.
Qed.
...
Lemma set_nth_id (e : seq T) (i : nat) (a x : T) :
  i < size e -> (set_nth x e i (nth a e i)) = e.
Proof.
...
Lemma pathE (T : eqType) (leT : rel T) (x : T) (s : seq T) : 
  transitive leT ->
  path.path leT x s = (path.sorted leT s) && (all (fun y => leT x y) s). 
Proof.
rewrite /transitive => trans_leT.
apply/idP/andP; last first.
move=> [sorted_s /allP leT_xs]; first by rewrite path.path_min_sorted.
move=> path_xs; split.
exact: (@path.path_sorted _ _ x _).
apply/allP.
move: x path_xs.
elim: s.
rewrite //=.
move=> a s ih /= x /andP [leT_xa path_as] y.
rewrite in_cons.
move/orP => [/eqP -> // | y_in_s].
apply: ih => //.
move: y_in_s path_as.
case: s => // b s.
rewrite in_cons.
move/orP => [/eqP eq_yb | y_in_s] /=.
move/andP => [leT_ab ->].
rewrite andbT.
exact: (@trans_leT _ _ _ leT_xa).
move/andP => [leT_ab ->].
rewrite andbT.
exact: (@trans_leT _ _ _ leT_xa).
Qed.

Lemma ltr_sorted_uniq_ler (R : numDomainType) (s : seq R) : 
  path.sorted Num.Def.ltr s = uniq s && path.sorted Num.Def.ler s.
Proof.
case: s => //= n s; elim: s n => //= m s IHs n.
rewrite inE Num.Theory.ltr_def negb_or IHs -!andbA.
case sn: (n \in s); last by rewrite eq_sym; do !bool_congr.
rewrite andbF.
apply/and5P=> [[ne_nm lenm _ _ le_ms]]; case/negP: ne_nm.
rewrite Num.Theory.eqr_le lenm andbT.
by apply: (allP (path.order_path_min (@Num.Theory.ler_trans R) le_ms)).
Qed.
...
Lemma seq_fset_nil : seq_fset [::] = (@fset0 K).
Proof. by apply/eqP; rewrite -cardfs_eq0 cardfsE. Qed.

Lemma seq_fset1 (x : K) : seq_fset [::x] = [fset x].
Proof. by apply/fsetP => y; rewrite in_fset1. Qed.

Lemma seq_fset_cat (s1 s2 : seq K) :
...
Lemma fset1D1 (x y : K) :
  [fset x] `\ y = if (y == x) then fset0 else [fset x].
Proof.
apply/fsetP=> b; have [->|] := altP (y =P x); rewrite ?inE;
by have [//->|]// := altP (b =P x); rewrite ?andbF // eq_sym => ->.
Qed.

(* Fact enum_fset0 : @enum_fset K fset0 = [::]. *)
(* Proof. by []. Qed. *)

(* Fact fset_sub_enum1 (x : K) : fset_sub_enum [fset x] = [::x]. *)
(* Proof. *)

(* Qed. *)

Fact enum_fset_seq_fset (s : seq K) : enum_fset (seq_fset s) =i s.
Proof. by move=> x; rewrite seq_fsetE. Qed.

Fact mem_enum_fset (s : {fset K}) : enum_fset s =i s.
Proof. by []. Qed.

Fact sort_keys1 (x : K) : sort_keys [::x] = [::x].
Proof.
rewrite /sort_keys ; set P := (X in choose X).
have : P (choose P [::x]) by apply/chooseP/perm_eq_refl.
by rewrite /P /= perm_eq_sym perm_eq1 ; move/eqP.
Qed.

Fact enum_fset1b (x : K) : enum_fset [fset x] = [::x].
Proof. rewrite /fset1 /seq_fset unlock ; exact: sort_keys1. Qed.

(* Lemma enum_fsetU (a : K) (s : {fset K}) : *)
(*   enum_fset (a |` s) =i if a \in s then (enum_fset s) else a::(enum_fset s). *)
(* Proof. *)
(* move: a. *)
(* move:s => [s i]. *)
...
Variable (I : choiceType) (s : seq I).

Check (pmap insub s).

(* Definition f (i : I) : option I := if i \in r then Some i else None. *)

(* Check ((pmap f) r). *)



(* Variables (T1 : eqType) (T2 : Type) (s : seq T1). *)
(* Variable (f : forall (t : T1), t \in s -> T2). *)

(* Fixpoint dep_map (s : seq T1) := if s is x :: s' then (f )::(dep_map s') else [::]. *)

(* Variable (s : seq K). *)
(* Check (seq_fset s). *)
(* Variable (t : seq (seq_fset s)). *)

(* Check (map (@fsval K (seq_fset s))). *)

(* Lemma topr : (map (@fsval K (seq_fset s)) t) = s. *)
(* Proof. *)
(* apply/eq_from_nth. *)
(* rewrite size_map. *)
(* rewrite //=. *)
...
(* Lemma in_join_fset (disp : unit) (x : K) (U : {fset {fset K}}) : *)
(*  x \in (\join_(s <- enum_fset U) s)%O. *)

(* have h : x \in (\join_(i <- enum_fset (interval_inf_b a |` aux_cells1 a s)) i)%O. *)
...
Variables (R : idomainType) (p q : {poly R}).

Lemma det_castmx (n n' : nat) (pr : (n = n') * (n = n')) (A : 'M[R]_n) : 
  \det (castmx pr A) = \det A.
Proof.
move: pr => [pr1 pr2].
move: pr1 pr2 A; case: _ / => pr A.
apply: eq_bigr => i _ ; congr (_ * _).
by apply: eq_bigr => j _ ; rewrite castmxE !cast_ord_id.
Qed.

Fact row_mx_eq0 (V : zmodType) (m n1 n2 n3 : nat) (A1 : 'M[V]_(m, n1)) 
                                               (A2 : 'M[V]_(m, n2)) : 
(row_mx A1 A2 == 0) = (A1 == 0) && (A2 == 0).
Proof.
rewrite -row_mx0; apply/idP/idP. 
move/eqP/eq_row_mx; first by move=> [] -> ->; rewrite !eqxx.
